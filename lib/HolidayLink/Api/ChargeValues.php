...
class ChargeValues extends Model {
  ...
  /**
   * Retrieve all charge values matching the $params
   *
   * @param  array $params
   * @param  Credentials $credentials API credentials
   *
   * @return self  the retrieved charge values
   */
  public static function search (array $params = null, Credentials $credentials = null) {
    ...
    $allowedParams = array(
      ...
      'status' => 1,
      'calculation_type' => 1,
      'calculation_timing' => 1,
      'calculation_per_item' => 1,
    );
    ...
    $call = new XmlCall($credentials);
    $sxe = $call->execute('charge-values/search', 'GET', array_intersect_key($params, $allowedParams));
    self::setTotalPageCount($call->getTotalPageCount());
