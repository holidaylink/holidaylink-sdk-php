...
class AccommodationUnitType extends Model {
  ...
  public static function updateSingle ($code, array $params = [], array $data= [], Credentials $credentials = null) {
    ...
    $sxe = $call->execute('accommodation-unit-types/' . $code, 'PUT', array_intersect_key($params, $allowedParams),
      $data);
  ...
  /**
   * Delete single accommodation type matching the $code filter
   *
   * @param  string $code
   * @param  Credentials $credentials API credentials
   *
  ...
  public static function deleteSingle ($code, Credentials $credentials = null) {
    ...
    $response = $call->execute('accommodation-types/' . $code, 'DELETE');
