...
class MediaDescriptions extends Model {
  ...
  /**
   * Retrieve all media-descriptions matching the $params
   *
   * @param  array $params
   * @param  Credentials $credentials API credentials
   *
   * @return self  the retrieved media-descriptions
   */
  public static function search (array $params = null, Credentials $credentials = null) {
    ...
    $call = new XmlCall($credentials);
    $sxe = $call->execute('media-descriptions/search', 'GET', array_intersect_key($params, $allowedParams));
    self::setTotalPageCount($call->getTotalPageCount());
